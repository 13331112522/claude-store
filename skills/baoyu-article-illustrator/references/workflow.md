...
**Execution choice**:
- If multiple illustrations already have saved prompt files and the task is now plain generation, prefer batch generation using the ListenHub API (iterate through prompt files sequentially)
...
**Default: Use ListenHub image-gen API**.

Image generation uses the ListenHub image-gen skill:
- **Auth**: `Bearer $LISTENHUB_API_KEY` (requires `LISTENHUB_API_KEY` env var)
- **Default model**: `gemini-3-pro-image-preview`
- **Fallback model**: `gemini-3.1-flash-image-preview` (faster, supports extreme aspect ratios)
- **API Reference**: Read `~/.claude/skills/image-gen/shared/api-image.md` for full details
- **Authentication**: Read `~/.claude/skills/image-gen/shared/authentication.md` for key setup

**API Endpoints** (try in order):

| Priority | Base URL | Note |
|----------|----------|------|
| Primary | `https://api.marswave.ai/openapi/v1` | Global CDN, reliable from all regions including China |
| Fallback | `https://api.labnana.com/openapi/v1` | Direct endpoint, may be blocked in some regions |

**Prompt extraction from files**:
```bash
# Extract content after YAML frontmatter (reliable)
PROMPT=$(awk '/^---$/{n++; next} n>=2' "{output-dir}/prompts/NN-{type}-{slug}.md")
```

Read the prompt file content and use it as the `prompt` field in the API request. Output images to the configured output directory.

### 5.2.1 Batch Generation Strategy ⚠️ IMPORTANT

Generate images **sequentially in small batches (1-2 at a time)**, NOT all in parallel:

```
For N illustrations:
  Batch 1: Image 1-2 → verify both succeed → proceed
  Batch 2: Image 3-4 → verify both succeed → proceed
  ...

On error:
  - Connection timeout → switch endpoint, retry once
  - 429 rate limit → wait 15s, retry (max 3)
  - 26004 insufficient credits → STOP, report progress, save remaining prompts
  - 29003 empty prompt → check frontmatter extraction
```

**Progress reporting**: After each batch:
```
Generated: X/N
- NN-{type}-{slug}.png ✓ ({size} bytes)
[or]
- NN-{type}-{slug}.png ✗ FAILED: {reason}
```
