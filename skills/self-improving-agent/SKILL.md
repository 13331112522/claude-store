...
- **Hooks Integration**: Auto-triggers on skill events (before\_start, after\_complete, on\_error)
...
|-------|---------|--------|
| **before\_start** | Any skill starts | Log session start |
| **after\_complete** | Any skill completes | Extract patterns, update skills |
| **on\_error** | Bash returns non-zero exit | Capture error context, trigger self-correction |
...
## Self-Correction (on\_error hook)
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.claude/skills/self-improving-agent/hooks/pre-tool.sh \"$TOOL_NAME\" \"$TOOL_INPUT\""
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.claude/skills/self-improving-agent/hooks/post-bash.sh \"$TOOL_OUTPUT\" \"$EXIT_CODE\""
          }
        ]
      }
    ],
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.claude/skills/self-improving-agent/hooks/session-end.sh"
          }
        ]
      }
    ]
